...
public interface EntityService<T extends BaseEntity> {
    ...
    static <T extends BaseEntity> BiFunction<CriteriaBuilder, Root<T>, Predicate> and(BiFunction<CriteriaBuilder, Root<T>, Predicate> first, BiFunction<CriteriaBuilder, Root<T>, Predicate> second) {
        return (cb, root) -> cb.and(first.apply(cb, root), second.apply(cb, root));
    }

    static <T extends BaseEntity> BiFunction<CriteriaBuilder, Root<T>, Predicate> or(BiFunction<CriteriaBuilder, Root<T>, Predicate> first, BiFunction<CriteriaBuilder, Root<T>, Predicate> second) {
        return (cb, root) -> cb.or(first.apply(cb, root), second.apply(cb, root));
    }
    static <T extends BaseEntity> BiFunction<CriteriaBuilder, Root<T>, Predicate> not(BiFunction<CriteriaBuilder, Root<T>, Predicate> condition) {
