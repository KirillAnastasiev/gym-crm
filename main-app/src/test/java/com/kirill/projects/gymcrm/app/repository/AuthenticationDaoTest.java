...
class AuthenticationDaoTest {
    ...
    void testChangePasswordForUsername() {
        ...
        var actualResult = authenticationDao.findUserByUsername(existingUsername, false)
    ...
    @Test
    @DisplayName("Test of the method findUserByUsername - should return user with given username with security data when withSecurity is true")
    void testFindUserByUsername_positive_withSecurityData() {
        // given
        var username = "John.Doe";

        // when
        var actualResult = authenticationDao.findUserByUsername(username, true);

        // then
        assertThat(actualResult).isNotNull();
        assertThat(actualResult).isPresent();
        assertThat(actualResult.get().getUsername()).isEqualTo(username);
        assertThat(actualResult.get().getSecurity()).isNotNull();
    }

    @Test
    @DisplayName("Test of the method findUserByUsername - should return empty optional if user with given username does not exist")
    void testFindUserByUsername_negative_unknownUsername() {
        // given
        var username = "Unknown.Username";

        // when
        var actualResult = authenticationDao.findUserByUsername(username, false);
