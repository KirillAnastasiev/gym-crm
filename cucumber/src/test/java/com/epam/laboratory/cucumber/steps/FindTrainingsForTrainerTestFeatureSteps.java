...
public class FindTrainingsForTrainerTestFeatureSteps {
    ...
    // ==================== FIND TRAINING FOR TRAINER TEST STEPS SUCCESSFUL ====================

    @When("The user attempts to find trainings for trainer {string}")
    public void the_user_attempts_to_find_trainings_for_trainer(String username) {
        contextHolder.put(USERNAME_KEY, username);
    }

    @When("Send a {string} request to the find trainings for trainer endpoint {string}")
    public void send_a_request_to_the_find_trainings_for_trainer_endpoint(String httpMethod, String endpoint) throws IOException, InterruptedException {
    ...
    @Then("The user receives a response containing a list of trainings for trainer {string}")
    public void the_user_receives_a_response_containing_a_list_of_trainings_for_trainer(String username) {
        ...
        AssertionsForInterfaceTypes.assertThat(trainings).isNotNull();
        AssertionsForInterfaceTypes.assertThat(trainings).isNotEmpty();
        trainings.forEach(training -> assertThat(training).extracting(Training::trainerUsername).isEqualTo(username));
    }
}
